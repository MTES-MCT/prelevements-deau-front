...
export const campaignIds = {
  ...
  secondMeter: '97777777-7777-4777-8777-777777777777', usage: '98888888-8888-4888-8888-888888888888',
  collector: '99999999-9999-4999-8999-999999999999', declaration: '90000000-0000-4000-8000-000000000001'
}
const states = new Map()
const uses = [{id: campaignIds.usage, kind: 'SUB_USAGE', code: '2A', label: 'Aspersion'}]
...
export async function handleCampaignFixtureRequest(request, send, response) {
  ...
  const permissions = {canManage: admin, canReadResults: admin || collector, canRespond: !admin && !collector, canDelete: admin, canOpen: admin}
  const campaign = () => ({id: campaignIds.campaign, name: state.name, status: state.status, opensOn: '2026-09-01', closesOn: '2026-12-31', type: 'DROPT_INDEX_NEEDS_2026_2027', collecteurUserId: campaignIds.collector, collecteur, updatedAt: `2026-09-24T00:00:0${state.revision}Z`, exploitationIds: [campaignIds.exploitation], progress: {total: 1, submitted: state.submitted ? 1 : 0, drafts: state.draft ? 1 : 0, remaining: state.submitted ? 0 : 1}, permissions})
  ...
  if (pathname === `/api/campaigns/${campaignIds.campaign}/responses`) return ok({items: [item()], total: 1, page: 1, pageSize: 25})
