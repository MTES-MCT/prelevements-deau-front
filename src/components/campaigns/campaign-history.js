...
export const CampaignReceipt = ({submission, context, kind}) => {
  ...
  return (
    <article id='campaign-submission-receipt' className='border border-gray-200 bg-white p-4'>
      ...
      </> : (snapshot.needs || []).map(need => <p key={`${need.targetId}-${need.periodId}`} className='text-sm'>{pointName(need.targetId)} · {context.campaign.periods.find(period => period.id === need.periodId)?.label || 'Période'} : {need.requestedVolume === null || need.requestedVolume === undefined || need.requestedVolume === '' ? 'Volume non renseigné' : `${need.requestedVolume} m³`}</p>)}
  ...
  return (
    <details className='fr-mb-3w'>
      ...
      <CampaignCard title='Réponses transmises'>
        ...
        {items.length > 0 && <CampaignField label='Réponse à consulter' value={selectedId} options={items.map(item => ({value: item.id, label: campaignDate(item.submittedAt, true, context.campaign.timezone)}))} onChange={setSelectedId} />}
