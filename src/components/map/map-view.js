...
const MapView = ({
  ...
  const [loadedMapStyle, setLoadedMapStyle] = useState(() => getInitialMapStyle(mapStyle))
  ...
  useEffect(() => {
    const bundledStyle = getInitialMapStyle(mapStyle)
    if (bundledStyle) {
      setLoadedMapStyle(current => current?.name === mapStyle ? current : bundledStyle)
      return undefined
    }
    ...
    const updateMapStyle = async () => {
      try {
        const style = await loadMapStyle(mapStyle)
        if (active) {
          setLoadedMapStyle(style)
        }
      } catch (error) {
        console.error(`Impossible de charger le fond de carte « ${mapStyle} ».`, error)
      }
    }
  ...
  useEffect(() => {
    if (!mapContainerRef.current || loadedMapStyle?.name !== mapStyle) {
      return
    }
  ...
  }, [loadedMapStyle, mapStyle, options.hash, options.cooperativeGestures, options.maxZoom, recenterControl, showNavigationControls])
  ...
  }, [loadedMapStyle, mapStyle, options.hash, preferUsageName, recenterControl, visiblePoints, visibleUsageKeys])
  ...
  }, [centerSelectedPointOnChange, loadedMapStyle, mapStyle, selectedPoint, selectedPointRequestKey])
  ...
  }, [highlightedPoint, loadedMapStyle, mapStyle, selectedPoint])
  ...
  }, [handleRecenter, loadedMapStyle, recenterRequestKey, visiblePoints.length])

  return (
    <Box className='flex h-full w-full relative'>
      <div ref={mapContainerRef} className='flex h-full w-full' />
      {loadedMapStyle?.name !== mapStyle && (
